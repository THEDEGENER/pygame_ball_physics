...
class Droplet():
    count = 0
    def __init__(self, screen, area=40, GRAVITY=200, COR=0.8):
        self.screen = screen
        self.HEIGHT = screen.get_height()
        self.WIDTH = screen.get_width()
        self.area = area
        self.drop_pos = pygame.Vector2(self.WIDTH / 2, self.area)
        self.vy = 0
        self.vx = 200
        self.dt = 0
    ...
    def draw(self):
        ...
        self.circle = pygame.draw.circle(self.screen, self.colour(), self.drop_pos, self.area)
    ...
    def update(self):
        ...
        if self.start:
            ...
            if self.drop_pos.y >= self.HEIGHT - self.area:
                self.drop_pos.y = self.HEIGHT - self.area  # Prevent sinking
                self.vy = -self.vy * self.COR  # Apply COR to Y velocity
                self.vx *= 0.95  # Simulate friction on the ground

            # Collision with right wall
            if self.drop_pos.x >= self.WIDTH - self.area:
                self.drop_pos.x = self.WIDTH - self.area  # Prevent going off-screen
                self.vx = -self.vx * self.COR  # Apply COR to X velocity

            # Collision with left wall
            if self.drop_pos.x <= self.area:
                self.drop_pos.x = self.area  # Prevent going off-screen
                self.vx = -self.vx * self.COR  # Apply COR to X velocity
    ...
    def init_placement(self):
        self.drop_pos.x += ((3 * self.area * self.count) - 1)
